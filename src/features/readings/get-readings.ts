export const getReadings = async (ymd : string, lectionary : string = 'rclsunday') : Promise<{
  readings: Entry[];
  date: string;
  lectionary: string;
}> => {
  const [y, m, d] = ymd.split('-'),
        url = urlFromLectionary(lectionary, y, m, d),
...
function urlFromLectionary(lectionary : string, y : string, m : string, d : string) : string {
